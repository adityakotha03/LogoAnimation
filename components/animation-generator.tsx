...
interface AnimationGeneratorProps { analysis: SvgAnalysis | null; svgLayers: Layer[]; svgContent: string; }

declare global { interface Window { anime: any; logoAnimation: any; } }
  ...
  const executeAnimationCode = useCallback(
    (code: string) => {
      ...
      if (!window.anime) {
        ...
        toast({ title: "Animation Error", description: "anime.js not loaded. Please try again.", variant: "destructive" });
        return;
      }
      if (!previewRef.current) {
      ...
      animeInstance.timeline = function (opts: any) {
      ...
      try {
        ...
        console.log(`Found ${testElements.length} SVG elements with IDs:`, 
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            <Button 
              ...
              className="flex-1"
            >
              {isGenerating ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Generating...</> : "Generate Animation"}
            </Button>
          </div>
